...
def cworkout(g6k, tracer, kappa, blocksize, cvp_start_dim, dim4free_min=0,              # Main parameters
    ...
    :param verbose: Print cworkout steps (with timing and quality) information on the standard
    ...
    with tracer.context(("cworkout", "beta:%d f:%d" % (blocksize, dim4free_min))):
        for f in fs:
            flast = f
            timestart = time.time()

            sys.stdout.flush()
            if(blocksize - f >= cvp_start_dim):
                pumpcvp(g6k,tracer, kappa, blocksize, f, verbose=True,  goal_r0 = goal_r0, down_sieve = True, min_cvp_dim = 70)
            else:
                pump(g6k, tracer, kappa, blocksize, f, goal_r0=goal_r0, **pump_params)
            # pumpcvp(g6k, tracer, kappa, blocksize, f, goal_r0=goal_r0, **pump_params)
